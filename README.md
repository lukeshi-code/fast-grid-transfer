...
- Data region: 324 x 76 cells.
- Encoding: default 4 colors, 2 bits per data cell, 6156 raw bytes per visual frame/grid. High-speed mode uses 8 colors, 3 bits per data cell, 9234 raw bytes per visual frame/grid.
...
- Capture threading: when supported, `MediaStreamTrackProcessor` reads real `VideoFrame` objects directly from the screen-capture track and transfers them to `decoder/frame-preprocess-worker.js`. The worker performs canvas scaling, pixel readback, and visual fingerprinting through `OffscreenCanvas`, with a latest-frame queue to avoid capture backlog. Browsers without direct track processing fall back to `requestVideoFrameCallback` plus `ImageBitmap`, then to main-thread canvas capture.
