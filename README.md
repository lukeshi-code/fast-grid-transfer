...
The encoder renders binary frames as two parallel 360 x 112 visual grids with 324 x 108 segmented data rows, four AprilTag 3 tag36h11 fiducial markers per grid, per-frame color calibration bars, and sync lines. The segmented layout uses rows between the corner markers without changing grid dimensions or pixels per cell. A Web Worker packages the selected file or folder stream, gzip-compresses it when useful, and encodes independent 16 MB RFC6330 RaptorQ chunks. The decoder captures the encoder tab/window, detects the AprilTags through a vendored WASM detector, samples each data grid through a homography, restores the RaptorQ chunks, verifies SHA-256, and downloads the file.
...
- Visual grid: 360 x 112 cells.
- Data region: 324 x 108 segmented cells; calibration and sync rows remain reserved.
- Encoding: default 4 colors, 2 bits per data cell, 8748 raw bytes and 8692 payload bytes per visual frame/grid. High-speed mode uses 8 colors, 3 bits per data cell, 13122 raw bytes and 13066 payload bytes per grid.
...
- Frame payload: raw frame bytes minus a 56-byte binary visual-frame header.
...
- Compression: for sources up to 32 MiB, the encoder tries browser `CompressionStream('gzip')` and transmits gzip bytes only when they are smaller than the original stream; the decoder restores with `DecompressionStream`. Larger sources use a bounded-memory raw stream so loading does not require full-file copies.
...
- Frame integrity: CRC32C checksum per visual frame packet.
- File integrity: SHA-256 in the stream metadata, verified after reconstruction.
- Transfer mode: RaptorQ encoding runs in independent 16 MB chunks. The encoder caches at most two packet chunks and the decoder releases each RaptorQ decoder after that chunk is recovered, preventing very large transfers from creating one enormous packet array.
- Folder mode: files are packaged as a TAR archive generated by the encoder worker.
- Encode threading: `encoder/fast-grid-encoder-worker.js` prepares the stream and builds visual frames. Large sources are SHA-256 hashed in 4 MiB reads and each 16 MiB RaptorQ block is read only when needed.
- Capture threading: capture requests a maximum of 1920 x 1080 at 60 fps. When supported, `MediaStreamTrackProcessor` reads real `VideoFrame` objects directly and disables the high-resolution preview while `decoder/frame-preprocess-worker.js` performs scaling and readback through `OffscreenCanvas`. Compatibility paths restore the live preview and fall back to `requestVideoFrameCallback` plus `ImageBitmap`, then main-thread canvas capture.
...
- Duplicate capture handling: once the ROI is locked, matching capture fingerprints are coalesced before worker decoding. One backup capture is retained until both grids pass CRC, then recent verified fingerprints and visual-frame keys are kept in bounded caches.
...
decoder/frame-preprocess-worker.js  Off-main-thread capture preprocessing
